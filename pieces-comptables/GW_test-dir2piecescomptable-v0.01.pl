...
#	my $rows2insert;
	if ( -f $File::Find::name ) {
	 	...
	 	if ( mimetype($File::Find::name) eq 'application/pdf' ) {
			...
			# PB how to deal with autoincrement field
			my @record_pc=();
			push @record_pc,$nb_pdf;
			push @record_pc,$md5;
#			push @record_pc,$nb_pdf,$md5;
			print join(",",@record_pc),"\n";
			my $rs 		= $schema->resultset('Piececomptable');
			my $record 	= $schema->resultset('Piececomptable')->new_result({});
			my $source 	= $schema->source('Piececomptable');
			my @fields 	= $source->columns;
			print join(",",@fields),"\n";
			
			print Dumper(@record_pc);
			
			my $res_insert = $schema->resultset('Piececomptable')->populate([[ @fields], [ @record_pc ]]);			

#			my $res_insert = $schema->resultset('Piececomptable')->populate([ [qw/piececomptable_id piececomptable_md5/ ], [ @record_pc ] ]);			
		}	
	}
=begin comment
...
#	my $VAR1 = '1';
#	my $VAR2 = 'f73d5d3f2aeba5f0fdb244299dd151a0';
...
#	print join(",",@fields),"\n";
 	...
 	my @test_record = (['h73d5d3f2aeba5f0fdb244299dd151a0']);

 	my $test_insert = $schema->resultset('Piececomptable')->populate([[qw/piececomptable_md5/], @test_record]);		